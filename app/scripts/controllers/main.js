...
//     localStorageService.clearAll();
      ...
      $scope.setInitialState = function(reset) {
          ...
          $scope.steps = localStorageService.get('steps') ? localStorageService.get('steps') : [
                ...
                {'title': 'Rank Values', 'description': 'The values may carry different weight as well. Keeping with jobs example, a desire to reduce commute time might be relatively more important than wages, so commute time should be higher in the list.', 'data': [], 'multiple': false, 'dependent': true},
                ...
                {'title': 'Find Optimal Decision', 'description': 'Here is your ranking! You may find that the choice doesn\'t line up with your intuition. This is OK! One added advantage of this process is it forces us to confront the conflict that often exists between what we think should prefer and what we actually do. Keeping with the job example, we may feel that we should not be as concerned about salary, but in truth, we do actually want to be paid more; it can be valuable to identify these preferences. Feel free to go back and adjust things.', 'data': [], 'dependent': true, 'multiple': true}
